<template>
  <div class="stock-management">
    ...
    <div
      v-if="chartData && chartData.length && chartLabels && chartLabels.length"
    >
      <h4>{{ chartTitle }}</h4>
      <line-chart
        :chartData="chartData"
    ...
    <div class="stock-alert">
      ...
      <input
        ...
        @change="checkStockAlert"
      />

      <div v-if="lowStockProducts.length">
        <h5>
          Voici les produits qui possèdent moins de {{ stockThreshold }} stock :
        </h5>
        <ul>
        ...
        <tbody>
          <tr v-for="product in filteredProducts" :key="product.id">
            ...
            <td>
              <input
                type="number"
                v-model.number="product.newAlertStock"
                min="0"
                style="width: 80px"
              />
            </td>
...
const fetchProducts = async () => {
  ...
  try {
    ...
    products.value = responseBody.map((product) => ({
      ...
      newAlertStock: product.alert_stock,
...
const checkStockAlert = () => {
  lowStockProducts.value = products.value.filter(
    (product) => product.stock < stockThreshold.value
  );
};
...
const editStock = async (product) => {
  if (
    ...
    product.newAlertStock !== product.alert_stock
  ) {
    if (
      !isNaN(product.newStock) &&
      Number.isInteger(product.newStock) &&
      product.newStock >= 0 &&
      !isNaN(product.newAlertStock) &&
      Number.isInteger(product.newAlertStock) &&
      product.newAlertStock >= 0
    ) {
      try {
        ...
        product.alert_stock = result.alert_stock;
      ...
      alert(
        "Veuillez entrer un nombre entier valide pour le stock et le stock d'alerte."
      );
    }
  }
};

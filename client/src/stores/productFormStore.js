...
export const useProductFormStore = defineStore("productForm", () => {
  ...
  const schema = z.object({
    ...
    discounted_price: z
      ...
      .refine((val) => /^\d+([.,]\d{1,2})?$/.test(val), {
        message:
          "Le prix promotionnel doit être un nombre positif avec ou sans centimes",
      }),
    ...
    alert_stock: z
      .number()
      .int()
      .nonnegative("Le stock d'alerte doit être un entier positif"),
  ...
  function setFormData(data) {
    ...
    Object.assign(formData, {
      ...
      discounted_price:
        data.discounted_price != null ? data.discounted_price.toString() : "0",
      character:
      ...
      image_preview_url: data.image_preview
        ...
        : "",
      image_gallery_urls: data.image_gallery
        ? data.image_gallery.map(ensureAbsoluteUrl)
        : ["", "", "", ""],
  ...
  watch(
    ...
    async (newCharacterId) => {
      if (newCharacterId) {
        ...
        if (character) {
          const universe = universes.value.find(
            (u) => u.id === character.universe
          );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      formData.discounted_price = formData.discounted_price
        ? parseFloat(formData.discounted_price.replace(",", "."))
        : 0;
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          setFlashMessage(
            "La référence existe déjà, veuillez en choisir une autre",
            "error"
          );
        } else {
          setFlashMessage(
            `Erreur: ${response.status} - ${errorMessage.error}`,
            "error"
          );
        }
      ...
      setFlashMessage("Le produit a été mis à jour avec succès", "success");
    } catch (error) {
  ...
  async function handleCreate() {
    try {
      ...
      setFlashMessage("Le produit a été créé avec succès", "success");
    } catch (error) {
      if (error instanceof z.ZodError) {
        error.errors.forEach((err) => {
          errors[err.path[0]] = err.message;

<template>
  <div class="carousel">
    <div class="carousel-item" v-if="currentMedia">
      <img :src="getImageUrl(currentMedia.url)" alt="carousel media" />
    </div>
    <button
      class="call-to-action"
      v-if="currentMedia && currentMedia.link"
      @click="goToLink"
    >
      Voir le produit
    </button>
...
const goToLink = () => {
  if (currentMedia.value && currentMedia.value.link) {
    router.push(currentMedia.value.link);
  }
};
...
watch(
  () => props.mediaItems,
  (newVal) => {
    if (newVal.length <= currentIndex.value) {

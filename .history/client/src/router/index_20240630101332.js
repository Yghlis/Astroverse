...
import jwtDecode from "jwt-decode"; // Utilisation de l'import par défaut
...
router.beforeEach((to, from, next) => {
  ...
  if (token) {
    try {
      const decoded = jwtDecode(token); // Utiliser l'import par défaut correctement ici
  ...
  if (mustChangePassword && to.name !== "ChangePassword") {
    next({ name: "ChangePassword", params: { userId: decoded.userId, token } });
  } else if (

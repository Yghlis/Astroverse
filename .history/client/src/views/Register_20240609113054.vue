  ...
  methods: {
    async register() {
      try {
        ...
        console.log('Register response:', data); // Ceci loggue la réponse du serveur

        if (response.ok) {
          // Si l'inscription est réussie, rediriger vers la page d'accueil
          this.$router.push('/');
        } else {
          // Si l'inscription échoue, afficher un message d'erreur (tu pourrais avoir un state pour ça)

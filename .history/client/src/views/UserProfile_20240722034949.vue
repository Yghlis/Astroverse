<template>
  <div class="profile-container">
    ...
    <h2>Mes Univers Suivis</h2>
    <TheCarousel>
      <universCard
        v-for="universe in displayUniverses"
        :key="universe.id"
        :title="universe.name"
        :color1="universe.color1"
        :color2="universe.color2"
        :colorText="universe.colorText"
        :link="'/universe/' + universe.id"
      ></universCard>
    </TheCarousel>
...
import universCard from client/src/components/landingPage/UniversCard.vue
...
onMounted(() => {
  ...
  universeStore.fetchFollowedUniverses(id);
...
const followedUniverses = computed(() => universeStore.followedUniverses);
...
// Watcher pour mettre à jour les univers suivis affichés lorsque les données changent
watch(
  followedUniverses,
  (newVal) => {
    displayUniverses.value = newVal;
  },
  { immediate: true }
);

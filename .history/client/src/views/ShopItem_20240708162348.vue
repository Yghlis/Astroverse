...
const handleNotification = async () => {
  ...
  const userId = "ca8667a0-870d-4de1-9aff-591369a0dee5"; // Replace with the actual user ID

  if (notification.value) {
    await productStore.followProduct(userId, product.value.id);
  } else {
    await productStore.unfollowProduct(userId, product.value.id);
  }
};

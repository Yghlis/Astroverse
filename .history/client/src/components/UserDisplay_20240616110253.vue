<template>
  ...
  <!-- Affichage des messages flash avec classe conditionnelle pour l'animation -->
  <p v-if="flashMessage" class="flash-message" :class="{ 'active': flashMessage }">{{ flashMessage }}</p>
...
const loginHandler = async () => {
  ...
  if (!userEmail.value || !userPassword.value) {
    setFlashMessage('Veuillez entrer une adresse e-mail et un mot de passe.', 3000); // Automatically clears after 3 seconds
    return;
  }
  ...
  if (response.ok) {
    ...
    setFlashMessage('Connexion réussie ! Bienvenue.', 3000); // Automatically clears after 3 seconds
  } else {
    const errorData = await response.json();
    setFlashMessage(errorData.message || 'Erreur de connexion', 3000); // Automatically clears after 3 seconds
  }
};
...
const logoutHandler = () => {
  ...
  setFlashMessage('Déconnexion réussie.', 3000); // Automatically clears after 3 seconds
};

const sendResetEmail = async () => {
  if (!userEmail.value) {
    setFlashMessage('Veuillez entrer votre email.', 3000); // Automatically clears after 3 seconds
    return;
  }
  ...
  if (response.ok) {
    setFlashMessage('Un email de réinitialisation a été envoyé si votre email est enregistré.', 3000); // Automatically clears after 3 seconds
    passwordResetRequested.value = false;
  } else {
    const errorData = await response.json();
    setFlashMessage(errorData.message || 'Erreur lors de l\'envoi de l\'email de réinitialisation.', 3000); // Automatically clears after 3 seconds
  }
};

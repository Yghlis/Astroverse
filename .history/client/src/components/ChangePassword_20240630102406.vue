  ...
  const changePassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Mettre à jour mustChangePassword à false
        localStorage.removeItem('jwt'); // Retirer l'ancien JWT
        const loginResponse = await fetch('http://localhost:8000/auth/login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: route.params.email, password: newPassword.value })
        });
  
        if (loginResponse.ok) {
          const data = await loginResponse.json();
          localStorage.setItem('jwt', data.token); // Stocker le nouveau JWT
          setTimeout(() => {
            router.push('/');
          }, 2000); // Rediriger vers la page principale après 2 secondes
        } else {
          errorMessage.value = 'Erreur lors de la mise à jour de la session';
        }
      } else {

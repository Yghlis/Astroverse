<template>
  ...
  Pour s'assurer que le champ `roles` est pris en compte correctement dans le formulaire de création d'utilisateur, nous devons adapter le store `userFormStore` et le composant `ModalCreate.vue`. Voici les modifications nécessaires :
  
  ### Adaptation du store `userFormStore`
  
  1. **Initialisation correcte de `roles` comme un tableau vide.**
  2. **Validation de `roles` pour s'assurer qu'au moins un rôle soit sélectionné.**

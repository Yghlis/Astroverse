...
const loginHandler = async () => {
  ...
  if (response.ok) {
    ...
    setFlashMessage('Connexion réussie ! Bienvenue.', 'success');
  } else {
    const errorData = await response.json();
    if (errorData.message.includes('locked')) {
      setFlashMessage('Le compte sur lequel vous essayez de vous connecter a été bloqué temporairement. Veuillez réessayer plus tard.', 'error');
    } else {
      setFlashMessage(errorData.message || 'Erreur de connexion', 'error');
    }
  }
};
...
const sendResetEmail = async () => {
  if (!userEmail.value) {
    setFlashMessage('Veuillez entrer votre email.', 'error');
    return;
  }
  ...
  if (response.ok) {
    setFlashMessage('Un email de réinitialisation a été envoyé si votre email est enregistré.', 'success');
    passwordResetRequested.value = false;
    setTimeout(() => flashMessage.value = '', 3000); // Cache le message après 3 secondes
  } else {
    const errorData = await response.json();
    setFlashMessage(errorData.message || 'Erreur lors de l\'envoi de l\'email de réinitialisation.', 'error');
    setTimeout(() => flashMessage.value = '', 3000); // Cache le message après 3 secondes
  }
};

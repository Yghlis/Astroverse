...
const onSubmit = async () => {
  console.log('Form data before submit:', formData.value); // Debug logformData.value.details = {
...
watch(() => props.selectedRow, (newRow) => {
  ...
  if (props.currentDataType === 'characters' && newRow) {
    ...
    tagsInput.value = newRow.tags ? newRow.tags.join(', ') : '';
    if (newRow.details) {
...
onMounted(() => {
  ...
  console.log('Initial selectedRow:', props.selectedRow);
  if (props.currentDataType === 'users' && props.selectedRow) {
    console.log('Setting form data for user on mount:', props.selectedRow);
    userFormStore.setFormData({
      user_id: props.selectedRow.user_id,
      first_name: props.selectedRow.first_name,
      last_name: props.selectedRow.last_name,
      email: props.selectedRow.email,
      password_hash: props.selectedRow.password_hash,
      phone_number: props.selectedRow.phone_number,
      address: {
        street: props.selectedRow.address?.street || '',
        city: props.selectedRow.address?.city || '',
        postal_code: props.selectedRow.address?.postal_code || '',
        country: props.selectedRow.address?.country || ''
      },
      roles: props.selectedRow.roles || []
    });
  }
  onMounted(async () => {

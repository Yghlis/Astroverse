<template>
  ...
  <h2 v-else-if="passwordExpired">Votre mot de passe a expiré, veuillez le réinitialiser</h2>
  ...
  <Transition name="slide" mode="out-in">
    <div v-if="!userLoggedIn && !passwordExpired">
...
const passwordExpired = ref(false); // Ajout de l'état pour passwordExpired
const userEmail = ref('');
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.mustChangePassword) {
        ...
        passwordExpired.value = data.passwordExpired; // Mise à jour de l'état du mot de passe
        if (passwordExpired.value) {
          setFlashMessage('Votre mot de passe a expiré, veuillez le réinitialiser.', 'error');
          console.log('Votre mot de passe a expiré, veuillez le réinitialiser.');
        } else {

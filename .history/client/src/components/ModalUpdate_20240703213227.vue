...
watch(() => props.selectedRow, (newRow) => {
  ...
  if (props.currentDataType === 'characters' && newRow) {
    ...
    const parsedTags = Array.isArray(newRow.tags) ? newRow.tags.map(tag => tag.replace(/[\[\]"]/g, '').trim()).join(', ') : '';
    console.log('Setting tags input:', parsedTags);
    tagsInput.value = parsedTags;

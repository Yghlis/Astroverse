<template>
  ...
  const formattedPrice = computed({
    get() {
      return formData.value.price.toString().replace('.', ',');
    },
    set(value) {
      formData.value.price = parseFloat(value.replace(',', '.'));
    }
  });
  
  const formattedDiscountedPrice = computed({
    get() {
      return formData.value.discounted_price.toString().replace('.', ',');
    },
    set(value) {
      formData.value.discounted_price = parseFloat(value.replace(',', '.'));
    }
  });

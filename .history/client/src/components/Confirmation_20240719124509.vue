<template>
    <div class="confirmation">
      <div class="confirmation-content">
        ...
        <div v-if="order.orderedProducts && order.orderedProducts.length">
          <h3>Détails de la commande :</h3>
          <ul>
            <li v-for="product in order.orderedProducts" :key="product.id">
              <img :src="product.image_preview" alt="Product Image" />
              <p>{{ product.title }}</p>
              <p>Prix: {{ product.price }}€</p>
              <p>Quantité: {{ product.OrderProduct.quantity }}</p>
            </li>
          </ul>
          <p>Taxe: {{ order.tax }}€</p>

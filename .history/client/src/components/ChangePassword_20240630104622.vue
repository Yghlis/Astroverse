    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!data.mustChangePassword) {
          // Update the token if necessary and navigate away from the change password page
          localStorage.setItem('jwt', data.token); // Assuming you might want to update the JWT as well
          setTimeout(() => {
            router.push('/');

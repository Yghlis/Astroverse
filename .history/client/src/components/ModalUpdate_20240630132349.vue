...
import { useUserFormStore } from '../stores/userFormStore';  // Importer le store des utilisateurs
import { ref, computed, watch, onMounted, reactive } from 'vue';

const universeFormStore = useUniverseFormStore();
const characterFormStore = useCharacterFormStore();
const productFormStore = useProductFormStore();
const userFormStore = useUserFormStore();  // Utiliser le store des utilisateurs
...
const formData = computed(() => {
  ...
  if (props.currentDataType === 'users') return userFormStore.formData;  // Ajouter la condition pour les utilisateurs
});
...
const errors = computed(() => {
  ...
  if (props.currentDataType === 'users') return userFormStore.errors;  // Ajouter la condition pour les utilisateurs
});
...
const isSubmitting = computed(() => {
  ...
  if (props.currentDataType === 'users') return userFormStore.isSubmitting;  // Ajouter la condition pour les utilisateurs
});
...
const onSubmit = async () => {
  ...
  if (props.currentDataType === 'universes') {
    ...
    capitalizeEmail();
    await userFormStore.handleSubmit();  // Ajouter la soumission pour les utilisateurs
  }
};
...
watch(() => props.selectedRow, (newRow) => {
  if (props.currentDataType === 'characters' && newRow) {
    ...
    userFormStore.setFormData(newRow);  // Ajouter la mise à jour des données de formulaire pour les utilisateurs
  }
});

...
onMounted(() => {
  ...
  console.log('Initial selectedRow:', props.selectedRow); // Log initial selectedRow
});
...
watch(() => props.selectedRow, (newRow) => {
  console.log('watch triggered for selectedRow:', newRow); 
  if (props.currentDataType === 'characters' && newRow) {
    ...
    userFormStore.setFormData({
      ...newRow,
      address: {
        ...userFormStore.formData.address,
        ...newRow.address
      },
      roles: newRow.roles || []
    });
    console.log('User form data after setting:', userFormStore.formData);
  }
});
</script>

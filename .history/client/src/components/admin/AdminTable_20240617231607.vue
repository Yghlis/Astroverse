<template>
    <div>
      ...
      <confirm-modal v-if="showConfirmModal" @confirm="deleteRow" @cancel="cancelDelete" />
    </div>
  </template>
    ...
    setup(props) {
      ...
      const filteredData = computed(() => {
        return props.data.filter(row =>
          props.columns.some(column => row[column].toString().toLowerCase().includes(searchQuery.value.toLowerCase()))
        );
      });
      ...
      const deleteRow = () => {
        // Logique de suppression ici (requête HTTP)

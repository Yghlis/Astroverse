<template>
  ...
  const sortedData = computed(() => {
    ...
    return [...filteredData.value].sort((a, b) => {
      const result = a[sortedColumn.value] > b[sortedColumn.value] ? 1 : -1;
      return sortOrder.value === 'asc' ? result : -1 * result;
    });
  });
  ...
  const deleteRow = () => {
    // Émettez un événement pour notifier le parent de supprimer la ligne

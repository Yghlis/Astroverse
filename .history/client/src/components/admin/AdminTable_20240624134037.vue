<template>
  <div>
    ...
    <table>
      <thead>
        <tr>
          <th v-for="(displayName, key) in displayColumns" :key="key" @click="sortTable(key)">
            {{ displayName }}
            <span>{{ sortedColumn === key ? (sortOrder === 'asc' ? '▲' : '▼') : '' }}</span>
          </th>
          <th>Actions</th>
        </tr>
      </thead>
...
const filteredData = computed(() => {
  return props.data.filter(row =>
    Object.keys(props.columns).some(column => 
      row[column] && row[column].toString().toLowerCase().includes(searchQuery.value.toLowerCase())
    )
  );
});
...
const exportCSV = () => {
  const csvContent = [
    ...
    ...props.data.map(row => Object.keys(props.columns).map(column => row[column]).join(','))
  ].join('\n');

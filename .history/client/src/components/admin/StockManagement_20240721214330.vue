<template>
    <div class="stock-management">
      ...
      <div class="controls">
        ...
        <input type="number" id="stockThreshold" v-model.number="stockThreshold" />
      </div>
      ...
      <div v-if="stockAlert" class="alert">
        <p>Warning: The stock for the selected product is below {{ stockThreshold }} units!</p>
      </div>
    </div>
  </template>
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      if (responseBody && responseBody.data && responseBody.labels) {
        ...
        // Vérifier le stock actuel et mettre à jour l'alerte de stock
        const currentStock = responseBody.data[responseBody.data.length - 1];
        stockAlert.value = currentStock < stockThreshold.value;
      } else {

...
const downloadInvoice = async (row) => {
  ...
  // Add company details
  doc.setFontSize(12);
  doc.text("Mon Entreprise", 14, 15);
  doc.text("Adresse de l'entreprise", 14, 20);
  doc.text("Numéro de téléphone", 14, 25);
  doc.text("Adresse e-mail de mon entreprise", 14, 30);
  doc.text("Numéro SIRET : XXX XXX XXX", 14, 35);
  doc.text("Numéro de TVA intracommunautaire : FRXX XXX XXX XXX", 14, 40);

  // Add invoice title
  doc.setFontSize(18);
  doc.text("Facture", 14, 50);

  // Add invoice details
  doc.setFontSize(12);
  doc.text(`Facture n° : ${row.id}`, 14, 60);
  doc.text(`Date de facturation : ${new Date(row.createdAt).toLocaleDateString()}`, 14, 66);
  doc.text(`Nom de l'acheteur : ${row.firstName} ${row.lastName}`, 14, 72);
  doc.text(`Adresse de facturation : ${row.billingAddress}`, 14, 78);
...
const downloadInvoice = async (row) => {
  ...
  // Add user details
  doc.setFontSize(12);
  doc.text(`Nom: ${row.firstName} ${row.lastName}`, 14, 28);

  // Add invoice details
  doc.text(`Numéro de commande: ${row.id}`, 14, 34);
  doc.text(`Date de commande: ${new Date().toLocaleDateString()}`, 14, 40);
  ...
  const taxAmount = (row.totalPrice * 0.20).toFixed(2);
  const totalPrice = (row.totalPrice * 1.20).toFixed(2);

  doc.setFontSize(12);
  doc.text(`Prix HT: ${row.totalPrice.toFixed(2)} €`, 14, doc.autoTable.previous.finalY + 10);
  doc.text(`TVA (20%): ${taxAmount} €`, 14, doc.autoTable.previous.finalY + 16);
  doc.text(`Prix total TTC: ${totalPrice} €`, 14, doc.autoTable.previous.finalY + 22);

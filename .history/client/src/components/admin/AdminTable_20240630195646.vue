<template>
  <div>
    ...
    <table>
      <thead>
        <tr>
          <!-- Enlever la case à cocher de l'entête des colonnes -->
          <th></th>
    ...
    <div class="table-controls">
      <button @click="openCreateModal">Créer</button> <!-- Bouton Créer ajouté ici -->
      <button @click="deleteSelectedRows" :disabled="selectedRows.length === 0">Supprimer sélection</button>
    </div>
  </div>
</template>
...
const deleteSelectedRows = async () => {
  try {
    for (const rowId of selectedRows.value) {
      const url = `http://localhost:8000/${props.currentDataType}/${rowId}`;
      const response = await fetch(url, { method: 'DELETE' });
      if (!response.ok) {
        const errorMessage = await response.text();
        throw new Error(`Erreur: ${response.status} - ${errorMessage}`);

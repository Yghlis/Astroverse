    ...
    <table>
      ...
      <tbody>
        <tr v-for="row in paginatedData" :key="row.id">
          ...
          <td v-for="column in columns" :key="column" v-html="renderCell(row, column)"></td>
...
const renderCell = (row, column) => {
  if (['color1', 'color2', 'colorText'].includes(column)) {
    return `<div class="color-box" style="background-color: ${row[column]};"></div>${row[column]}`;
  }

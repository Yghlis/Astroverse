...
const downloadInvoice = (row) => {
  ...
  // Add invoice details
  doc.setFontSize(12);
  doc.text(`Numéro de commande: ${row.id}`, 14, 32);
  doc.text(`Date de commande: ${new Date().toLocaleDateString()}`, 14, 38);
  ...
  // Add total price
  doc.setFontSize(12);
  doc.text(`Prix total: ${row.totalPrice} €`, 14, doc.autoTable.previous.finalY + 10);

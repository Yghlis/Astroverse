...
const downloadInvoice = async (row) => {
  ...
  // Split the billing address to correctly format city and postal code
  const addressParts = row.billingAddress.split(', ');
  const streetAddress = addressParts[0];
  const cityAndPostal = addressParts[1] ? addressParts[1].split(' ') : ['', ''];
  const city = cityAndPostal.slice(0, -1).join(' ');
  const postalCode = cityAndPostal.slice(-1)[0];

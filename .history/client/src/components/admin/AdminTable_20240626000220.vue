<template>
  <div>
    ...
    <Modal v-if="showModal" @close="closeModal" :currentDataType="currentDataType" :modalType="modalType" :width="'90%'" :height="'auto'">
      <template #header>
...
const { setFormData: setUniverseFormData } = universeFormStore;
const { setFormData: setCharacterFormData } = characterFormStore;
...
const openModal = (type, row) => {
  ...
  if (props.currentDataType === 'universes' && type === 'edit') {
    setUniverseFormData(row); 
  }
  if (props.currentDataType === 'characters' && type === 'edit') {
    setCharacterFormData(row); 
  }

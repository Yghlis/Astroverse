<template>
    ...
    const config = {
      ...
      options: {
        ...
        scales: {
          ...
          yAxes: selectedCurves.value.map((_, index) => ({
            type: 'linear',
            display: true,
            ...
            // grid lines settings
            grid: {
              drawOnChartArea: index % 2 === 0, // only draw grid lines on one axis
            },
          })),
        },
        plugins: {
  ...
  const updateChart = () => {
    if (chartInstance.value) {
      ...
      chartInstance.value.data.datasets = selectedCurves.value.map((curveId, index) => ({
        label: allCurves.value.find((curve) => curve.id === curveId)?.label || "Unknown",
        data: getChartData(selectedTimeframe.value, curveId),
        fill: false,
        borderColor: `rgb(${75 + index * 20}, 192, 192)`,
      ...
      chartInstance.value.options.scales.yAxes = selectedCurves.value.map((_, index) => ({
        type: 'linear',
        display: true,
        position: index % 2 === 0 ? 'left' : 'right',
        id: `y-axis-${index}`,
        grid: {
          drawOnChartArea: index % 2 === 0,
        },
      }));
  ...
  watch([selectedTimeframe, selectedCurves], () => {
    updateChart();
  });
  </script>

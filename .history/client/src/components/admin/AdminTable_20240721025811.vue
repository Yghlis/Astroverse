...
const downloadInvoice = async (row) => {
  ...
  img.src = logo;
  img.onload = () => {
    doc.addImage(img, 'PNG', 14, 10, 50, 15); // Position x, y and dimensions of the logo
    ...
    doc.setFontSize(12);
    doc.text("Astroverse", 14, 40);
    doc.text("34 rue astrobouse, 75012 PARIS", 14, 45);
    doc.text("0656554455", 14, 50);
    doc.text("Astroverse-Admin@gmail.com", 14, 55);
    doc.text("Numéro SIRET : 123 321 213", 14, 60);
    doc.text("Numéro de TVA intracommunautaire : FR72 934 710 566", 14, 65);

    // Add customer details on the right
    doc.text(`Nom de l'acheteur :`, 140, 40);
    doc.text(`${row.firstName} ${row.lastName}`, 140, 45);
    const billingAddress = row.billingAddress.replace(/, /g, '\n');
    doc.text(`Adresse de facturation :`, 140, 50);
    doc.text(billingAddress, 140, 55);

    // Add invoice details below company and customer details
    doc.text(`Facture n° : ${row.id}`, 14, 75);
    doc.text(`Date de facturation : ${new Date(row.createdAt).toLocaleDateString()}`, 14, 80);

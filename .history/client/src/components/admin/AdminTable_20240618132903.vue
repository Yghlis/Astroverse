<template>
  ...
  import EditModal from '../crud/EditModal.vue'; // Assurez-vous de créer un composant EditModal
  ...
  const editData = ref(null); // Ajouter cette ligne
  const editableFields = ref([]); // Ajouter cette ligne
  ...
  const edit = (row) => {
    if (props.currentDataType === 'universes') {
      ...
      editableFields.value = [
        'title', 'brand', 'price', 'discounted_price', 'is_promotion', 'description', 'stock',
        'number_of_purchases', 'number_of_favorites', 'rating', 'image_preview', 'image_gallery',
        'character', 'universe', 'reference', 'details', 'tags', 'availability_status', 'views_count'
      ];
    }

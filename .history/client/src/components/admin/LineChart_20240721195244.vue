<template>
  ...
  const createChart = () => {
    ...
    const data = {
      ...
      datasets: toRaw(selectedCurves.value).map((curveId, index) => ({
    ...
    const config = {
      ...
      options: {
        ...
        scales: {
          yAxes: toRaw(selectedCurves.value).map((curveId, index) => ({
            type: 'linear',
            display: true,
  ...
  const updateChart = () => {
    if (chartInstance.value) {
      ...
      chartInstance.value.data.datasets = toRaw(selectedCurves.value).map((curveId, index) => ({
        label: allCurves.value.find((curve) => curve.id === curveId)?.label || "Unknown",
        data: getChartData(selectedTimeframe.value, curveId),
        fill: false,
        borderColor: `rgb(${75 + index * 20}, 192, 192)`,
      ...
      chartInstance.value.options.scales.yAxes = toRaw(selectedCurves.value).map((curveId, index) => ({
        type: 'linear',
        display: true,
        position: index % 2 === 0 ? 'left' : 'right',
        id: `y-axis-${index}`,

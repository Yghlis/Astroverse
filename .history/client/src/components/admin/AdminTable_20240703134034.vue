<template>
  <div>
    ...
    <ModalCreate v-if="showCreateModal" @close="closeCreateModal" @create-success="handleCreateSuccess" :currentDataType="currentDataType" :width="'90%'" :height="'auto'">
      <template #header>
        <h3>Créer</h3>
      </template>
    </ModalCreate>
...
const emit = defineEmits(['edit', 'view', 'row-deleted', 'create-success']);
...
const renderCell = (row, column) => {
  if (column === 'universe' && row[column]) {
    const universe = universes.value.find(univ => univ.id === row[column]);
    return universe ? universe.name : row[column];
  }
  return row[column];
};
...
const handleCreateSuccess = (newData) => {
  ...
  fetchUniverses(); // Refresh the universes data
  searchQuery.value = ''; // Reset the search to display all data
  currentPage.value = 1; // Reset to the first page
  setFlashMessage('Création réussie', 'success');
};
</script>

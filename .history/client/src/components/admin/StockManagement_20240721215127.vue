<template>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="product in products" :key="product.id">
              ...
              <td>{{ product.stock }}</td>
              <td><button @click="editStock(product)">Modifier le stock</button></td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  const editStock = (product) => {
    const newStock = prompt(`Modifier le stock pour ${product.name}:`, product.stock);
    if (newStock !== null) {
      product.stock = parseInt(newStock, 10);
      // You can add code here to update the stock on the server if needed
    }
  };

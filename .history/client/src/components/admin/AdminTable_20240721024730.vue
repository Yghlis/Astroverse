...
const downloadInvoice = async (row) => {
  ...
  doc.setFontSize(12);
  doc.text("Mon Entreprise", 14, 25);
  doc.text("Adresse de l'entreprise", 14, 30);
  doc.text("Numéro de téléphone", 14, 35);
  doc.text("Adresse e-mail de mon entreprise", 14, 40);
  doc.text("Numéro SIRET : XXX XXX XXX", 14, 45);
  doc.text("Numéro de TVA intracommunautaire : FRXX XXX XXX XXX", 14, 50);

  // Add customer details on the right
  doc.text(`Nom de l'acheteur :`, 140, 25);
  doc.text(`${row.firstName} ${row.lastName}`, 140, 30);
  
  // Ensure billingAddress is defined and correctly formatted
  const billingAddress = row.billingAddress || 'Adresse non fournie';
  const addressParts = billingAddress.split(', ');
  const streetAddress = addressParts[0] || 'Adresse non fournie';
  const cityAndPostal = addressParts[1] ? addressParts[1].split(' ') : ['', ''];
  const city = cityAndPostal.slice(0, -1).join(' ') || 'Ville non fournie';
  const postalCode = cityAndPostal.slice(-1)[0] || 'Code postal non fourni';

  doc.text(`Adresse de facturation :`, 140, 35);
  doc.text(`${streetAddress}`, 140, 40);
  doc.text(`${city} ${postalCode}`, 140, 45);

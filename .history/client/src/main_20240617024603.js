...
if (token) {
    try {
        ...
        localStorage.setItem('role', decoded.role); // Stocker le rôle pour une utilisation ultérieure
    } catch (e) {
        console.error("Erreur lors du décodage du JWT :", e);
        // Gérer l'erreur, par exemple en déconnectant l'utilisateur ou en supprimant un token invalide
    }
}
if (token) {
    const decoded = jwt_decode(token);

    ...
    async syncCart() {
      try {
        ...
        this.cartItems = data.items.map(item => ({
          ...
          discounted_price: item.discounted_price || item.product?.discounted_price || 0, // Ajout de discounted_price
          is_promotion: item.is_promotion || item.product?.is_promotion || false, // Ajout de is_promotion
        }));

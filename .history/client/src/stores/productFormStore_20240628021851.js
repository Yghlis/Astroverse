  ...
  async function handleSubmit() {
    try {
      ...
      formData.image_gallery.forEach((file, index) => {
        const imageGalleryInput = document.querySelector(`input[name="image_gallery"]`);
        if (imageGalleryInput && imageGalleryInput.files[index]) {
          formDataToSend.append('image_gallery', imageGalleryInput.files[index]);
        }
      });

  ...
  async function handleSubmit() {
    try {
      ...
      for (const key in formData) {
        if (formData.hasOwnProperty(key) && key !== 'image_gallery' && key !== 'image_preview') {
      ...
      formData.image_gallery.forEach((file, index) => {
        const imageGalleryInput = document.querySelector(`input[name="image_gallery[${index}]"]`);
        if (imageGalleryInput && imageGalleryInput.files[0]) {
          formDataToSend.append('image_gallery', imageGalleryInput.files[0]);
        }
      });

      console.log('FormData keys:', Array.from(formDataToSend.keys())); // Debugging log
      console.log('FormData values:', Array.from(formDataToSend.values())); // Debugging log

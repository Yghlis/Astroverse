...
export const useUniverseFormStore = defineStore('universeForm', () => {
  ...
  const followedUniverses = ref([]);  // Ajout de l'état pour les univers suivis
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/universes/${formData.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('jwt')}`
  ...
  return {
    ...
    followUniverse,
    unfollowUniverse,
    followedUniverses  // Assurez-vous que followedUniverses est exporté
  };
});

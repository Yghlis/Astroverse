    ...
    async syncCart() {
      try {
        ...
        this.cartItems = data.items.map(item => ({
          ...
          title: item.title || 'Unknown title', // Assurez-vous que chaque article a un titre
          price: item.price || 0, // Assurez-vous que chaque article a un prix
          image_gallery: item.image_gallery || [], // Assurez-vous que chaque article a une galerie d'images
        }));

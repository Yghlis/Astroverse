...
export const useCartStore = defineStore('cart', {
  ...
  actions: {
    ...
    async incrementItemQuantity(itemId) {
      ...
      if (itemToIncrement) {
        try {
          ...
          if (checkStockData.available) {
            // Stock suffisant, mettre à jour l'élément dans le panier côté serveur
            const addBasketResponse = await fetch('http://localhost:8000/basket', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
    ...
    async decrementItemQuantity(itemId) {
      ...
      if (itemToDecrement) {
        try {
          // Mettre à jour l'élément dans le panier côté serveur
          const removeBasketResponse = await fetch('http://localhost:8000/basket/decrement', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'session-id': localStorage.getItem('sessionId'),
            ...
            body: JSON.stringify({ productId: itemId, quantity: 1 }) // ici, on décrémente de 1
          });

          if (!removeBasketResponse.ok) {
            const errorText = await removeBasketResponse.text();
            console.error('Error decrementing product quantity in basket:', errorText);
            return;
          }
          ...
          console.error('Error decrementing product quantity in basket:', error);
        }
      }
    },

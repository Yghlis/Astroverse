export const useProductFormStore = defineStore('productForm', () => {
    ...
    async function handleSubmit() {
      try {
        ...
        if (document.querySelector('input[name="image_preview"]').files[0]) {
          formDataToSend.append('image_preview', document.querySelector('input[name="image_preview"]').files[0]);
        }
        formData.image_gallery.forEach((file, index) => {
          if (document.querySelector(`input[name="image_gallery[${index}]"]`).files[0]) {
            formDataToSend.append('image_gallery', document.querySelector(`input[name="image_gallery[${index}]"]`).files[0]);
          }
        });

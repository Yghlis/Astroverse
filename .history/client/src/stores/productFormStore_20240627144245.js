...
export const useProductFormStore = defineStore('productForm', () => {
  ...
  const schema = z.object({
    ...
    image_gallery: z.array(z.string().optional()).length(4, 'Il faut exactement 4 images pour la galerie'),
  ...
  const universes = ref([]);
  const imagePreviewFile = ref(null);
  const imageGalleryFiles = ref([null, null, null, null]);
  ...
  async function fetchCharacters() {
    try {
      const response = await fetch('http://localhost:8000/characters', { method: 'GET' });
  ...
  async function fetchUniverses() {
    try {
      const response = await fetch('http://localhost:8000/universes', { method: 'GET' });
  ...
  async function handleSubmit() {
    try {
      ...
      for (const key in formData) {
        if (key !== 'image_gallery' && key !== 'image_preview') {
          formDataToSend.append(key, formData[key]);
        }
      }
      if (imagePreviewFile.value) {
        formDataToSend.append('image_preview', imagePreviewFile.value);
      }
      imageGalleryFiles.value.forEach((file, index) => {
        if (file) {
          formDataToSend.append(`image_gallery`, file);
        }
      });
  ...
  function handleImageGalleryChange(event, index) {
    const file = event.target.files[0];
    if (file && file.type.startsWith('image/')) {
      imageGalleryFiles.value[index] = file;
      const reader = new FileReader();
      reader.onload = () => {

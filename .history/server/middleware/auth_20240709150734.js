...
export async function authenticateToken(req, res, next) {
    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, user) => {
        ...
        console.log('User authenticated:', user); // Ajout de log pour vérifier le contenu du token

        req.user = user;
        
        // Ajout de logs pour vérifier le contenu de req.user
        console.log('User object in request after assigning:', req.user);
        ...
        try {
            ...
            req.user.role = dbUser.roles[0]; // Ajout de la première rôle à req.user pour compatibilité avec requireRole

...
// Middleware pour générer un sessionId et l'ajouter à la requête
app.use((req, res, next) => {
  if (!req.headers['session-id']) {
    const sessionId = uuidv4();
    res.setHeader('session-id', sessionId);
    req.sessionId = sessionId;
    console.log('Generated new session ID:', sessionId); // Ajout de la vérification
  } else {
    req.sessionId = req.headers['session-id'];
    console.log('Existing session ID:', req.sessionId); // Ajout de la vérification
  }
  next();
});

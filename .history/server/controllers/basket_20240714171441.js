...
export const addToBasket = async (req, res) => {
  ...
  try {
    ...
    if (!basket) {
      ...
      basket.items = items; // Mettre à jour le champ items avec le tableau modifié
      basket.updatedAt = new Date();
      await basket.update({ items: items, updatedAt: new Date() }); // Assurez-vous d'utiliser `update` pour sauvegarder les modifications

...
export const addUser = async (req, res) => {
  ...
  const authenticatedUserRole = req.user.role; // Récupère le rôle de l'utilisateur authentifié
  ...
  try {
    ...
    const newUserData = {
      ...
      isEmailVerified: true, // Par défaut, chaque nouvel utilisateur a son email vérifié
      mustChangePassword: true, // Par défaut, chaque nouvel utilisateur doit changer son mot de passe
      lastPasswordChange: new Date()
    };

    // Si l'utilisateur authentifié est un admin, il peut définir des champs supplémentaires
    if (authenticatedUserRole === 'ROLE_ADMIN') {
      if (roles) {
        newUserData.roles = roles;
      }
      if (isEmailVerified !== undefined) {
        newUserData.isEmailVerified = isEmailVerified;
      }

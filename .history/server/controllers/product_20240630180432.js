...
const deleteUnusedFiles = (oldFiles, newFiles) => {
  oldFiles.forEach(file => {
    if (!newFiles.includes(file) && fs.existsSync(path.resolve('uploads', path.basename(file)))) {
      fs.unlinkSync(path.resolve('uploads', path.basename(file)));
    }
  });
};
...
export const updateProduct = async (req, res) => {
  try {
    ...
    try {
      ...
      if (image_preview !== oldImagePreview && fs.existsSync(path.resolve('uploads', path.basename(oldImagePreview)))) {
        fs.unlinkSync(path.resolve('uploads', path.basename(oldImagePreview)));
      }
...
export const deleteProduct = async (req, res) => {
  try {
    ...
    try {
      ...
      if (fs.existsSync(path.resolve('uploads', path.basename(product.image_preview)))) {
        fs.unlinkSync(path.resolve('uploads', path.basename(product.image_preview)));
      }
      deleteUnusedFiles(product.image_gallery, []);

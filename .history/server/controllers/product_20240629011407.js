...
export const updateProduct = async (req, res) => {
  try {
    ...
    const image_gallery = Array.isArray(req.files['image_gallery']) && req.files['image_gallery'].length > 0 
      ? req.files['image_gallery'].map(file => `${req.protocol}://${req.get('host')}/uploads/${file.filename}`)

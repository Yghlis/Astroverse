...
export const addCharacter = async (req, res) => {
  ...
  try {
    ...
    // Vérifiez si un personnage avec le même nom existe déjà dans le même univers
    const existingCharacter = await Character.findOne({
      where: { name, universe: universeRecord.id }
    });

    if (existingCharacter) {
      return res.status(409).json({ error: 'A character with the same name already exists in this universe' });
    }
...
export const updateCharacter = async (req, res) => {
  ...
  try {
    ...
    // Vérifiez si un autre personnage avec le même nom existe déjà dans le même univers
    const existingCharacter = await Character.findOne({
      where: {
        name,
        universe: universeRecord.id,

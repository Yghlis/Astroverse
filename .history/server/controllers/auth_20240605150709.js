...
async function sendVerificationEmail(user, req) {
    ...
    try {
        ...
        // Vous pourriez envisager de gérer cette erreur de manière plus robuste, par exemple en renvoyant une réponse d'erreur au client.
    }
}
...
const verifyEmail = async (req, res) => {
    ...
    const user = await User.findOne({
        ...
        emailVerificationExpires: { $gt: new Date() }  // Vérifiez que le token n'est pas expiré
    });
    ...
    user.emailVerificationExpires = undefined; // Nettoyez le token et la date d'expiration après vérification
    await user.save();
...
const postLogin = async (req, res) => {
    ...
    try {
        ...
        if (!user.isEmailVerified) {
            // Ici, vous pouvez retourner un message spécifique pour les utilisateurs non vérifiés
...
const postSignup = async (req, res) => {
    ...
    try {
        ...
        const user = new User({
            ...
            emailVerificationExpires: new Date(Date.now() + 10*60*1000)  // Ajouter le token de vérification ici
        });

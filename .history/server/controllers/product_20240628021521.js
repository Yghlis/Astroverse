...
export const updateProduct = async (req, res) => {
  try {
    ...
    // Vérifiez si les détails sont un objet valide
    let details;
    try {
      details = JSON.parse(req.body.details);
    } catch (error) {
      details = req.body.details; // Si ce n'est pas un JSON, on le garde tel quel
    }
    ...
    const image_gallery = Array.isArray(req.files['image_gallery']) ? req.files['image_gallery'].map(file => `${req.protocol}://${req.get('host')}/uploads/${file.filename}`) : [];

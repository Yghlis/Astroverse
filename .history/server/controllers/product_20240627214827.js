...
export const addProduct = async (req, res) => {
  try {
    ...
    const image_preview = req.files && req.files['image_preview'] ? req.files['image_preview'][0].filename : null;
    const image_gallery = req.files && req.files['image_gallery'] ? req.files['image_gallery'].map(file => file.filename) : [];
    ...
    try {
      ...
      const product = await Product.create({
        ...
        number_of_favorites,
        rating,
        image_preview: image_preview ? `${req.protocol}://${req.get('host')}/uploads/${image_preview}` : null,
        image_gallery: image_gallery.length ? image_gallery.map(filename => `${req.protocol}://${req.get('host')}/uploads/${filename}`) : [],

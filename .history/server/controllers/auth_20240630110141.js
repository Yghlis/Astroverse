...
const postLogin = async (req, res) => {
    ...
    try {
        ...
        // Générer le token avec les informations nécessaires
...
const postChangePassword = async (req, res) => {
    ...
    user.mustChangePassword = false; // Reset the flag to false
    await user.save();

    // Optionally, you can regenerate a token if necessary
    const token = jwt.sign(
        { userId: user.user_id, email: user.email, role: user.roles[0], mustChangePassword: user.mustChangePassword },
        process.env.JWT_SECRET,
        { expiresIn: '1h' }

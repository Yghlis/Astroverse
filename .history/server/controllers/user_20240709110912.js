...
export const updateUser = async (req, res) => {
  const { id } = req.params;
  const authenticatedUserId = req.user.userId;
  const { first_name, last_name, email, password_hash, phone_number, address, roles, isEmailVerified, mustChangePassword, resetPasswordReminder } = req.body;
  ...
  try {
    ...
    if (existingUser) {
      return res.sendStatus(400); // Email déjà utilisé
    }

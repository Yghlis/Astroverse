...
export const getProducts = async (filters) => {
  ...
  if (filters.characters && filters.characters.length > 0) {
    query.include[0].where.name = { [Op.in]: filters.characters };
  } else {
    delete query.include[0].where;
  }

  if (filters.universes && filters.universes.length > 0) {
    query.include[1].where.name = { [Op.in]: filters.universes };
  } else {
    delete query.include[1].where;
  }

  if (filters.ratings && filters.ratings.length > 0) {
    query.where.rating = { [Op.in]: filters.ratings };
  }

  if (filters.priceRange && filters.priceRange.min !== undefined && filters.priceRange.max !== undefined) {
    query.where[Op.or] = [
      { price: { [Op.between]: [filters.priceRange.min, filters.priceRange.max] }, discounted_price: { [Op.is]: null } },
      { discounted_price: { [Op.between]: [filters.priceRange.min, filters.priceRange.max] } }
    ];
  }
  ...
  try {
    ...
    if (Object.keys(filters).length === 0 || 
        (filters.characters.length === 0 && filters.universes.length === 0 && filters.ratings.length === 0 && filters.priceRange.min === 0 && filters.priceRange.max === Infinity && !filters.isPromotion)) {
      products = await Product.findAll({
        include: [
          { model: Character, attributes: ['id', 'name'] },
          { model: Universe, attributes: ['id', 'name'] }
        ]
      });
    } else {

...
export const deleteProduct = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    if (!id || !isUUIDValid(id)) {
      return res.status(400).end(); // Code 400 pour ID invalide
    }

    const product = await Product.findByPk(id, { transaction });
    if (!product) {
      await transaction.rollback();
      return res.status(404).end(); // Code 404 pour produit non trouvé
    }
    ...
    res.status(204).end(); // Code 204 pour suppression réussie
  } catch (error) {
    await transaction.rollback();

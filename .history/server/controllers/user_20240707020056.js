...
export const toggleNewsletterSubscription = async (req, res) => {
  ...
  if (id !== req.user.user_id && req.user.roles.indexOf('ROLE_ADMIN') === -1) {
  ...
  try {
    ...
    await user.save();
    res.status(200).json({ message: 'Subscription status updated', isSubscribedToNewsletter: user.isSubscribedToNewsletter });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

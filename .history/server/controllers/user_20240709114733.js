...
export const addUser = async (req, res) => {
  ...
  try {
    ...
    res.status.json(201).json(newUser);
  } catch (error) {
  ...
  try {
    ...
    if (toggleNewsletterSubscription !== undefined) {
      ...
      try {
        ...
        console.log(`Email sent to ${user.email} for ${user.isSubscribedToNewsletter ? 'subscription' : 'unsubscription'}`);
      } catch (emailError) {

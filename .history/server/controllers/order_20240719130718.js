...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    // Ajouter les produits à la commande en utilisant OrderProduct pour stocker la quantité
    await Promise.all(
  ...
  try {
    const order = await Order.findOne({
      ...
      include: [
        {
          model: Product,
          as: 'orderedProducts', // Utilisation de l'alias correct
          attributes: ['id', 'title', 'price', 'image_preview'],
          through: { attributes: ['quantity'] } // Inclure la quantité du produit dans la commande
        }
      ]
    });

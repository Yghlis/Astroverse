...
export const notifyProductChange = async (product) => {
  try {
    const { previousStock, previousPromotion, stock, is_promotion, title, id } = product;

    const followers = await Follow.findAll({ where: { productId: id }, include: [User] });
    console.log(`Found ${followers.length} followers for product ${title} with ID ${id}`);
    
    for (const follow of followers) {
      const user = follow.User;
      console.log(`Checking conditions for user ${user.email}`);
      
      console.log(`Previous stock: ${previousStock}, Current stock: ${stock}`);
      console.log(`Previous promotion: ${previousPromotion}, Current promotion: ${is_promotion}`);
      
      if (stock === 0) {
        console.log(`Notifying ${user.email} that product ${title} is out of stock`);
        const emailContent = `Le produit ${title} est en rupture de stock.`;
        await sendEmail(user.email, 'Produit en rupture de stock', emailContent);
      } else if (previousStock === 0 && stock > 0) {
        console.log(`Notifying ${user.email} that product ${title} is back in stock`);
        const emailContent = `Le produit ${title} est de nouveau en stock.`;
        await sendEmail(user.email, 'Produit de nouveau en stock', emailContent);
      }

      if (is_promotion && !previousPromotion) {
        console.log(`Notifying ${user.email} that product ${title} is now on promotion`);
        const emailContent = `Le produit ${title} est maintenant en promotion.`;
        await sendEmail(user.email, 'Produit en promotion', emailContent);
      } else if (!is_promotion && previousPromotion) {
        console.log(`Notifying ${user.email} that product ${title} is no longer on promotion`);
        const emailContent = `Le produit ${title} n'est plus en promotion.`;

...
const postLogin = async (req, res) => {
    ...
    const token = jwt.sign(
        { userId: user.user_id, email: user.email, role: user.roles[0] },
...
const postChangePassword = async (req, res) => {
    ...
    res.status(200).json({ 
        ...
        mustChangePassword: user.mustChangePassword // Ensure mustChangePassword is sent back in the response
    });
};

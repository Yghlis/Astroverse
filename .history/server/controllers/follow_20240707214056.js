...
export const notifyProductChange = async (product) => {
  try {
    ...
    for (const follow of followers) {
      ...
      if (product.stock === 0) {
        await sendEmail(user.email, 'Produit en rupture de stock', `Le produit ${product.title} est en rupture de stock.`);
      } else if (product.previous('stock') === 0 && product.stock > 0) {
        await sendEmail(user.email, 'Produit de nouveau en stock', `Le produit ${product.title} est de nouveau en stock.`);
      }

      if (product.is_promotion && product.previous('is_promotion') === false) {
        await sendEmail(user.email, 'Produit en promotion', `Le produit ${product.title} est maintenant en promotion.`);
      } else if (!product.is_promotion && product.previous('is_promotion') === true) {
        await sendEmail(user.email, 'Produit plus en promotion', `Le produit ${product.title} n'est plus en promotion.`);
      }
    }
  } catch (error) {
...
export const notifyNewProductInUniverse = async (product) => {
  try {
    ...
    for (const follow of followers) {
      const user = follow.User;
      await sendEmail(user.email, 'Nouveau produit dans un univers suivi', `Un nouveau produit ${product.title} a été ajouté dans l'univers que vous suivez.`);
    }
  } catch (error) {

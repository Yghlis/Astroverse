...
export const getUniverseById = async (req, res) => {
  ...
  try {
    ...
    if (!universe) {
      return res.sendStatus(404); // Code 404 pour indiquer que l'univers n'a pas été trouvé
    }
    res.status(200).json(universe); // Code 200 OK pour indiquer que la requête a réussi
  } catch (error) {
    res.sendStatus(500); // Code 500 pour une erreur interne du serveur
  }
};

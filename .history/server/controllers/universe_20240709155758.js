...
export const deleteUniverse = async (req, res) => {
  ...
  try {
    ...
    if (productsCount > 0) {
      await transaction.rollback();
      return res.sendStatus(400); // Mauvaise requête : l'univers a des produits associés
    }

    const charactersCount = await Character.count({ where: { universe: id }, transaction });
    if (charactersCount > 0) {
      await transaction.rollback();
      return res.sendStatus(400); // Mauvaise requête : l'univers a des personnages associés
    }

    await universe.destroy({ transaction });
    await transaction.commit();
    res.sendStatus(200); // OK : univers supprimé avec succès
  } catch (error) {
    await transaction.rollback();

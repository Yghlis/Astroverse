...
export const getProducts = async (req, res) => {
  ...
  const [minPrice, maxPrice] = priceRange ? priceRange.split('-x').map(Number) : [0, Infinity];
  const isPromotion = promotion === 'true';

  const filters = {};

  if (characterArray.length) {
    filters['character.name'] = { $in: characterArray };
  }

  if (universeArray.length) {
    filters['universe.name'] = { $in: universeArray };
  }

  if (ratingArray.length) {
    filters.rating = { $in: ratingArray };
  }

  if (priceRange) {
    filters.$or = [
      { price: { $gte: minPrice, $lte: maxPrice }, discounted_price: { $exists: false } },
  ...
  try {
    ...
    const products = await ProductMongo.find(filters);

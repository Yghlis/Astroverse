...
export const handleStripeWebhook = async (req, res) => {
  ...
  switch (event.type) {
    case 'payment_intent.succeeded':
      ...
      // Mettre à jour la commande en "En cours"
    ...
    case 'payment_intent.payment_failed':
      ...
      await Order.destroy({ where: { id: failedOrderId } });
...
cron.schedule('* * * * *', async () => {
  ...
  for (const order of pendingOrders) {
    console.log(`Order ${order.id} is still pending after 3 minutes. Marking as failed.`);
    await Order.update({ status: 'Échouée' }, { where: { id: order.id } });
  }
});

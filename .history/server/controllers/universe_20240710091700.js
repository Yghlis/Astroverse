...
export const followUniverse = async (req, res) => {
  console.log('Request user:', req.user); // Log de req.user
  const userId = req.user?.id; // Utiliser l'opérateur optionnel pour éviter les erreurs si req.user est undefined
  const { universeId } = req.params;

  if (!userId) {
    console.error('User ID is undefined');
    return res.sendStatus(400); // Erreur 400 si userId est undefined
  }

...
const runTests = async () => {
  ...
  const totalTests = 5; // Nombre total de tests
  ...
  // Test PATCH /orders/:orderId (mettre à jour le statut de la commande)
  try {
    const orderId = orderData.orderId;
    const patchOrderResponse = await fetch(`${serverUrl}/orders/${orderId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authToken}`,
      ...
      body: JSON.stringify({
        status: 'Expédiée', // Nouveau statut de la commande
      }),
    });

    const patchOrderData = await patchOrderResponse.json();
    assert.strictEqual(patchOrderResponse.status, 200);
    console.log('Statut de la commande mis à jour:', patchOrderData);
    successfulTests++;
  } catch (error) {
    console.error('Test "PATCH /orders/:orderId" échoué - PAS BON:', error);
  }
  ...
  console.log(`5. Test "PATCH /orders/:orderId" - ${successfulTests >= 5 ? 'BON' : 'PAS BON'}`);
};

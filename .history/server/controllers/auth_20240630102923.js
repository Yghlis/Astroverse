...
const postLogin = async (req, res) => {
    ...
    try {
        ...
        if (!user.isEmailVerified) {
            return res.status(401).json({ message: 'Your email address has not been verified. Please check your email for the verification link.' });
        }
        ...
        res.status(200).json({
            message: user.mustChangePassword ? 'Must change password' : 'Login successful',
            token: token,
            userId: user.user_id,
  ...
  const postChangePassword = async (req, res) => {
    ...
    user.mustChangePassword = false; // Mettre à jour l'indicateur mustChangePassword à false
    await user.save();
